...
def test_only_daily_weekend():
    result = calculate_reports(closing_days, daily_days, "2025-05-04", holidays)  # Sunday
    assert len(result) == 1
    assert result[0]["report_type"] == "daily"

def test_only_daily_holiday():
    result = calculate_reports(closing_days, daily_days, "2025-05-01", holidays)  # Holiday
    assert len(result) == 1
    assert result[0]["report_type"] == "daily"
